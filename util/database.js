...
// There are 2 ways of connecting to a SQL database: 1) Set up one connection to use for running queries. Should close connection once done with query. Downside: Must re-execute code to create the connection for every new query, and many queries (fetch data, write data, delete data); creating new connections all the time is inefficient, both in code and with regard to performance cost connecting to database. 2) Connection pool: better alternative
...
// The promise part allows using  promises when working with these connections
